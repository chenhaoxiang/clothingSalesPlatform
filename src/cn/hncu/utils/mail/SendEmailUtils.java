...
public class SendEmailUtils extends Thread{
	...
	private SendEmail send = null;

	public SendEmailUtils(SendEmail send) {
		this.send = send;
	}
	...
	public void run() {
		...
		try {
			...
			// 2收件人
			msg.setRecipient(RecipientType.TO, new InternetAddress(send.getEmail()));
			// 3邮件内容:主题、内容
			msg.setSubject(send.getName() + ",欢迎注册超级聚会账号,请点击链接激活账号");

			// StringBuilder是线程不安全的,但是速度快，这里因为只会有这个线程来访问，所以可以用这个
			StringBuilder sbd = new StringBuilder();
			sbd.append(send.getName() + "<br/>欢迎！请确认此邮件地址以激活您的账号。<br/>");
			//TODO 链接还未改
			sbd.append("<font color='red'><a href='http://localhost:8080/chaojijuhui/active/user?acode="
					+ send.getId() +"&userType="+send.getUserType() + "' target='_blank'");
			sbd.append(">立即激活</a></font><br/>");
			sbd.append("或者点击下面链接:<br/>");
			//TODO 链接还未改
			sbd.append("http://localhost:8080/chaojijuhui/active/user?acode="
					+ send.getId()+"&userType="+send.getUserType() + " <br/>");
			...
			System.out.println("给" + send.getEmail() + "发送邮件成功。");
